...
import { DatabaseResult, UserListItem, StakingDetailItem, StakingEarningDetailItem, DepositBUSDDetailItem, WithdrawRequestWithAddress } from '../../../libs/types';

class AdminModel {
  async getWithdrawRequest(): Promise<WithdrawRequestWithAddress[]> {
    return mockData.getAllWithdrawRequests();
  }

  async approveWithdrawRequest(data: { id: number; hash: string }): Promise<DatabaseResult> {
  ...
  async rejectWithdrawRequest(data: { request_id: number }): Promise<DatabaseResult> {
    return mockData.updateWithdrawRequest(data.request_id, {
      status: 2,
    });
  }

  async balanceUpdate(data: { user_id: number; token_amount: number }): Promise<DatabaseResult> {
  ...
  async getStakingDetail(): Promise<StakingDetailItem[]> {
  ...
  async getStakingEarningDetail(): Promise<StakingEarningDetailItem[]> {
  ...
  async getDepositBUSDDetail(): Promise<DepositBUSDDetailItem[]> {
