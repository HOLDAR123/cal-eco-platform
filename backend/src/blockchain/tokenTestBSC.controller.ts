...
import { TransferData } from '../../libs/types';
...
export const addressCheck = async (request: Request, response: Response): Promise<void> => {
  let address = request.params.address;
  let status = ethers.utils.isAddress(address);
...
export const transfer = async (data: TransferData): Promise<string> => {
  ...
  let tokenValue = ethers.utils.parseEther(amount.toString());
  ...
  try {
    ...
    if (token.div(ethers.BigNumber.from(10).pow(decimals)).lt(amount)) {
...
export const getTokenBalance = async (request: Request, response: Response): Promise<void> => {
  ...
  try {
    ...
    response.status(200).json({
      balance: token.div(ethers.BigNumber.from(10).pow(decimals)).toString(),
    });
  } catch (e) {
...
export const createWallet = async (request: Request, response: Response): Promise<void> => {
  try {
...
export const getTransactionByAddress = async (request: Request, response: Response): Promise<void> => {
  ...
  try {
    ...
    let transactions = resData.data.ethereum.transactions;
    response.status(200).json({
      transactions
    });
  } catch (e) {
